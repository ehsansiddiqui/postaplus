...
class home_m extends CI_Model {
    ...
    public function __construct(){
        parent::__construct();
        $this->load->database();
        $this->ordItemsTable = 'order_details';
    }
 ...
 $result = $this->db->get_where('shop_product', array('shop_product_id' =>$shop_product_id,'status'=>1));
    ...
    function get_gift_category(){
       ...
       $language_id = $this->session->userdata('language_id')?$this->session->userdata('language_id'):1;
          ...
          if($language_id == 2){
            $this->db->select('gift_category_id,ar_gift_cate_name as gift_cate_name,gift_cate_image,gift_category_status');
        }else{
            $this->db->select('gift_category_id,gift_cate_name,gift_cate_image,gift_category_status');   
        }      
...
LEFT JOIN user u ON (u.user_id = o.user_id) LEFT JOIN branch b ON (b.branch_id = o.branch_id) LEFT JOIN order_status os ON (os.order_status_id = o.status) WHERE o.user_id = $user_id AND o.payment_status = 'paid' ORDER BY o.order_id DESC";  
    ...
    public  function get_brands_price($brand_id){
        $result = $this->db->get_where('brand', array('brand_id'=>$brand_id));
        return $result->row();  
    } 
}
?>