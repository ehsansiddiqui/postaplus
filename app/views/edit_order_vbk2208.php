...
function fill_unit_select_box1($addons)
{
 ...
 {
    $output .='<option value="'.$row->addons_id.'">'.$row->addons_name.'</option>'; 
 }
 return $output;
}
?>
...
<!--                    <table id="POITable" class="table datatable">
    ...
    <th><button type="button" name="add" class="btn btn-success btn-sm add"><span class="glyphicon glyphicon-plus"></span></button></th>
        ...
        <td>     
            ...
            <?php 
             ...
             {
            ?>    
     <option value="<?=$row->product_type_id?>" <?php if($row->product_type_id == $details->product_type_id){ echo 'selected' ;} ?>><?=$row->types_name.'-->'.$row->gender.'-->'.$row->product_name?></option> 
            <?php 
             }
            ?>
        </select>
  ...
  <input type="text" name="item_price[]"  value="<?=$details->product_price?>" id="p<?=$i?>" onkeyup="sum1(<?=$i?>);" class="form-control item_price" /></td>
    <td>     
        ...
        <select name="item_addon[]" class="form-control" id="wow<?=$i?>" onchange="wow(<?=$i?>)" multiple>    
            <?php 
...
$(document).ready(function(){    
 ...
 $(document).on('click', '.add', function(){   
  ...
  html += '<td><input type="text" name="item_price[]" class="form-control item_price" value="" id="txt2'+i+'"  onkeyup="sum('+i+');" /></td>';
  html += '<td><select name="item_addon[]" class="form-control" id="addon'+i+'" onchange="getaddonPrice('+i+')"><option value="0">Nothing selected</option><?php echo fill_unit_select_box1($addons); ?></select></td>';
