...
<!--                            <tr>
    ...
    <?php endforeach; ?>
           ...
           <input type="hidden" id="price_package_id" />
...
$(document).ready(function(){
 ...
 $(document).on('click', '.add', function(){ 
  ...
  var price_package_id = $("#price_package_id").val();
  
  
               $.ajax({
                        type: "POST",
                        url: '<?= site_url('order/get_price_packages') ?>',
                        data: {price_package_ids: price_package_id},
                        cache: false,
                        success: function (r)
                        {
             ...
             if(JSON.parse(r).status == 100){
                 ...
                 // $price_packages ='';        
                $price_packages ='<option value="0" selected>Select Type</option>';
                $price_packages_list = '';
                $price_packages1 = '';
                
                    $.each(JSON.parse(r).res, function(index, value){
                        
                  $price_packages +='<option value="'+value.product_type_id+'">'+value.types_name+'->'+value.gender+'->'+value.product_name+'</option>';
                     $price_packages_list +='<li rel="'+index+'" class=""><a tabindex="0" class="" style=""><span class="text">'+value.customer_address+'</span><i class="glyphicon glyphicon-ok icon-ok check-mark"></i></a></li>';
                     //$price_packages1 = value.types_name+'->'+value.gender+'->'+value.product_name;
                    });
                ...
                $elm1 = $('#item_name'+i+'');
		$elm1.html($price_packages); 
                //$elm1.siblings().find('.dropdown-toggle').remove('.filter-option');
                //$elm1.siblings().find('.dropdown-toggle').html('<span class="filter-option pull-left">'+$price_packages1+'</span><span class="caret"></span>');
                 $elm1.siblings().children().siblings().find('.dropdown-menu').html($price_packages_list);
 ...
 //$('#item_name1')[0].onclick();
  ...
  var html = '';
  html += '<tr>';
  html += '<td><select name="item_name[]" class="form-control select" id="item_name'+i+'" onchange="getPrice('+i+')" data-live-search="true"></select></td>';
...
//                      var json = $.parseJSON(r); 
             ...
             if(JSON.parse(r).status == 100){
                 ...
                 $.each(JSON.parse(r).res, function(index,value){    
                   ...
                   $("#price_package_id").val(value.price_package_id);
                 });
